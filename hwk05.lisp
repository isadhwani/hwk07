...
We assume the property A1 holds true
Apply the formula for p = q: (!p or q) ^ (p or !q).
Substitute this formula into the given expression, resulting in ((p = q) = !p) = r, which becomes (v (^ q (! r)) (^ r (! q))).
Use the property A1 to prove the equivalence of the two formulas.
property A1 asserts that these two formulas are equivalent, so we have (== (== r (== (! p) (== q p))) (v (^ q (! r)) (^ r (! q))).
equivalence is established, and the proof is complete.
...
Assume Property A2 holds for all boolean inputs
Apply the formula for p !^ (q => p !v q) and simplify it to (v p (! q)).
Use "Property A2" to prove the equivalence of the two formulas.
"Property A2" asserts that these two formulas are equivalent, so we have (== (p !^ (=> (! p) (v q r))) (v p (! q))).
The equivalence is established, and the proof is complete.
...
Assume property A3 holds true for all boolean inputs
Start with the given expression: (3) p !^ (q => p !v q).
Apply the formula for XOR and the implications: (p and (not q)) or ((not p) and q).
Use "Property A3" to prove the equivalence.
Conclude that the two expressions are equivalent.
...
Assume property A4 holds true for all boolean inputs
Start with the given expression: (4) (a => b) ^ (c => a) <> (a !^ b) !v (!a ^ !c).
Apply the formulas for implications: ((not a) or b) ^ ((not c) or a) <> ((a and (not b)) or ((not a) and (not c))).
Apply the formula for XOR: ((not a) or b) ^ ((not c) or a) <> (a and (not b)) or ((not a) and (not c)).
Simplify to: (^ (! a) (! c)).
Use "Property A4" to prove the equivalence and conclude that the two expressions are equivalent.
...
; This is a data definition for the Boolean Formulas you have to
...
(definec BfEval (p :BoolFm a :assignment) :bool
  (match p
    ...
    ((q '<> r) (if (BfEval q a) (! (BfEval r a)) (BfEval r a)))
    ((q 'v r) (v (BfEval q a) (BfEval r a)))
    ((q '^ r) (^ (BfEval q a) (BfEval r a)))
    ((q '!v r) (! (v (BfEval q a) (BfEval r a))))
    (('! q) (! (BfEval q a)))))

(property BfEval/return (p :BoolFm a :assignment)
  (or (= (BfEval p a) t) (= (BfEval p a) nil)))

(property BfEval/len (p :BoolFm a :assignment)
  (= (len (BfEval p a)) 0))

(check (BfEval '(! (a !^ b)) '(a b)))
(check (BfEval '(a !v b) '()))
(check (BfEval '(a v b) '(b)))
(check (BfEval '(! (a ^ b)) '(b)))
(check (BfEval '(a <> b) '(a)))
(check (BfEval '(! (a <> b)) '(a b)))
(check (BfEval '(a <> b) '(b)))
...
(definec BoolFm->BoolFm1 (p :BoolFm) :BoolFm1
  (match p
    ...
    ((q e r) `(,(BoolFm->BoolFm1 q) ,e ,(BoolFm->BoolFm1 r)))
    (('! q) `(! ,(BoolFm->BoolFm1 q)))
    (& p)))

(property BoolFm->BoolFm1/len (p :BoolFm) 
  (>= (len p) (len (BoolFm->BoolFm1 p))))

(property BoolFm->BoolFm1/output-is-BoolFm (p :BoolFm) 
  (BoolFmp(BoolFm->BoolFm1 p)))


(check= (BoolFm->BoolFm1 '(! ((a ^ b) !v (c v d))) ) '(! ((a ^ b) !v (c v d))))
(check= (BoolFm->BoolFm1 '(! (a ^ (b v (c !^ d))))) '(! (a ^ (b v (! (c ^ d))))))
...
(property (p :BoolFm a :assignment)
  (equal (BfEval p a) (BfEval(BoolFm->BoolFm1 p) a)))
...
(definec BoolFm1->BoolFm2 (p :BoolFm1) :BoolFm2
  (match p
    ((q '<> r) `((,(BoolFm1->BoolFm2 q) v ,(BoolFm1->BoolFm2 r)) ^ 
                 (!(,(BoolFm1->BoolFm2 q) ^ ,(BoolFm1->BoolFm2 r)))))
    ((q e r) `(,(BoolFm1->BoolFm2 q) ,e ,(BoolFm1->BoolFm2 r)))
    (('! q) `(! ,(BoolFm1->BoolFm2 q)))
    (& p)))


(property BoolFm1->BoolFm2/len (p :BoolFm) 
  (>= (len p) (len (BoolFm->BoolFm1 p))))

(property BoolFm1->BoolFm2/output-is-BoolFm (p :BoolFm) 
  (BoolFmp(BoolFm->BoolFm1 p)))

(check= (BfEval (BoolFm1->BoolFm2 (BoolFm->BoolFm1 '(a <> (b !^ c)))) '(a c))
        (BfEval '(a <> (b !^ c)) '(a c)))


"Property 2"
(property (p :BoolFm1 a :assignment)
  (equal (BfEval p a) (BfEval (BoolFm1->BoolFm2 p) a)))
...
(definec BoolFm2->BoolFm3 (p :BoolFm2) :BoolFm3
  (match p
    ((q 'v r) `((,(BoolFm2->BoolFm3 q) !v ,(BoolFm2->BoolFm3 r)) !v 
                (,(BoolFm2->BoolFm3 q) !v ,(BoolFm2->BoolFm3 r))))
    ((q e r) `(,(BoolFm2->BoolFm3 q) ,e ,(BoolFm2->BoolFm3 r)))
    (('! q) `(! ,(BoolFm2->BoolFm3 q)))
    (& p)))

(property BoolFm2->BoolFm3/len (p :BoolFm) 
  (>= (len p) (len (BoolFm->BoolFm1 p))))

(property BoolFm2->BoolFm3/output-is-BoolFm (p :BoolFm) 
  (BoolFmp(BoolFm->BoolFm1 p)))


(check= (BoolFm2->BoolFm3 '(! (a ^ (b v c))))
        '(! (a ^ ((b !v c) !v (b !v c)))))
...
(property (p :BoolFm2 a :assignment)
  (equal (BfEval p a) (BfEval (BoolFm2->BoolFm3 p) a)))
...
(definec BoolFm3->BoolFm4 (p :BoolFm3) :BoolFm4
  (match p
    ((q '^ r) `((,(BoolFm3->BoolFm4 q) !v ,(BoolFm3->BoolFm4 q)) !v 
                (,(BoolFm3->BoolFm4 r) !v ,(BoolFm3->BoolFm4 r))))
    ((q e r) `(,(BoolFm3->BoolFm4 q) ,e ,(BoolFm3->BoolFm4 r)))
    (('! q) `(! ,(BoolFm3->BoolFm4 q)))
    (& p)))

(property BoolFm3->BoolFm4/len (p :BoolFm) 
  (>= (len p) (len (BoolFm->BoolFm1 p))))

(property BoolFm3->BoolFm4/output-is-BoolFm (p :BoolFm) 
  (BoolFmp(BoolFm->BoolFm1 p)))

(check= (BoolFm3->BoolFm4 '((x ^ y) !v (a ^ b))) 
        '(((x !v x) !v (y !v y)) !v ((a !v a) !v (b !v b))))
...
;('! (CONS '! (CONS (BOOLFM3->BOOLFM4 Q) 'NIL)))

"Property 4"
(property (p :BoolFm3 a :assignment)
  (equal (BfEval p a) (BfEval (BoolFm3->BoolFm4 p) a)))

; We already defined NorFm, so no need to define BoolFm5.

(definec BoolFm4->NorFm (p :BoolFm4) :NorFm
  (match p
    ((q e r) `(,(BoolFm4->NorFm q) ,e ,(BoolFm4->NorFm r)))
    (('! q) `(,(BoolFm4->NorFm q) !v ,(BoolFm4->NorFm q)))
    (& p)))

(check= (BoolFm4->NorFm '((! a) !v (! b))) '((a !v a) !v (b !v b)))

"Property 5"
(property (p :BoolFm4 a :assignment)
  (equal (BfEval p a) (BfEval (BoolFm4->NorFm p) a)))
...
(definec BoolFm->5NorFm (p :BoolFm) :NorFm
  (BoolFm4->NorFm 
   (BoolFm3->BoolFm4 (BoolFm2->BoolFm3 (BoolFm1->BoolFm2 (BoolFm->BoolFm1 p))))))

(property BoolFm->5NorFm/len (p :BoolFm) 
  (or (>= (len p) (len (BoolFm->5NorFm p)))
      (<= (len p) (len (BoolFm->5NorFm p)))))

(property BoolFm->5NorFm/output-is-BoolFm (p :BoolFm) 
  (BoolFmp(BoolFm->5NorFm p)))


(check= (BoolFm->5NorFm '(! ((a ^ b) v (a !^ (c <> d))))) '(((((a !v a) !v (b !v b))
   !v
   (((a !v a)
     !v
     (((((c !v d) !v (c !v d))
...
(check= (BoolFm->5NorFm '(a ^ b)) '((a !v a) !v (b !v b)))
(check= (BoolFm->5NorFm '(a <> b)) '((((a !v b) !v (a !v b))
  !v ((a !v b) !v (a !v b)))
 !v
 ((((a !v a) !v (b !v b))
   !v ((a !v a) !v (b !v b)))
...
(check= (BoolFm->5NorFm '(a !^ b)) '(((a !v a) !v (b !v b)) !v ((a !v a) !v (b !v b))))

(check= (BoolFm->5NorFm '(a v b)) '((a !v b) !v (a !v b)))

(check= (BoolFm->5NorFm '(a !v b)) '(a !v b))
...
"Property 6"
(property (p :BoolFm a :assignment)
  (equal (BfEval p a) (BfEval (BoolFm->5NorFm p) a)))
...
(definec BoolFm->NorFm (p :BoolFm) :NorFm
  (cond
   ((! (BoolFm1p p)) (BoolFm->NorFm (BoolFm->BoolFm1 p)))
   ((! (BoolFm2p p)) (BoolFm->NorFm (BoolFm1->BoolFm2 p)))
   ((! (BoolFm3p p)) (BoolFm->NorFm (BoolFm2->BoolFm3 p)))
   ((! (BoolFm4p p)) (BoolFm->NorFm (BoolFm3->BoolFm4 p)))
   ((! (NorFmp p)) (BoolFm->NorFm (BoolFm4->NorFm p)))
   (t p)))

;(property BoolFm->NorFm/output-is-BoolFm (p :BoolFm) 
 ; (NorFmp(BoolFm->NorFm p)))




(check= (BoolFm->NorFm '(! ((a ^ b) v (a !^ (c <> d))))) '(((((a !v a) !v (b !v b))
   !v
   (((a !v a)
     !v
     (((((c !v d) !v (c !v d))
...
"Property 7"
(property (p :BoolFm a :assignment)
  (equal (BfEval p a) (BfEval (BoolFm->NorFm p) a)))
...
(defconst *NorFM-large* '((a !v nil) !v nil))
...
"Property 8"
(property (x :bool)
  (iff (bfeval *NorCPFm-small* '()) (bfeval *NorFM-large* '())))
...
(defconst *NorFM-small* '(a !v nil))
(check (NorFMp *NorFM-small*))

(defconst *NorCPFm-large* '(((((a !v a) !v (a !v a))
   !v ((a !v a) !v (a !v a)))
  !v
  ((((a !v a) !v (a !v a))
    !v ((a !v a) !v (a !v a)))
